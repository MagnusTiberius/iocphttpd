...
void RequestParser::Parse()
{
	...
	while (token != token_t::ENDTOKEN)
	{
		printf(".");
		token = Token();
		if (token == RequestParser::token_t::GET || token == RequestParser::token_t::POST)
		{
		...
		if (token == RequestParser::token_t::URL)
		{
			item = new headermap_t{};
		...
		if (token == RequestParser::token_t::QUERYSTRING)
		{
			item = new headermap_t{};
		...
		if (token == RequestParser::token_t::PROPERTYNAME)
		{
			item = new headermap_t{};
...
int RequestParser::Token()
{
	...
	if (c1 != NULL)
	{
		...
		if (strcmp(d1, "GET") == 0)
		{
			m_token = d1;
			m_token = _strdup("GET");
			return RequestParser::token_t::GET;
		}
		if (strcmp(d1, "POST") == 0)
		{
			m_token = d1;
			m_token = _strdup("POST");
			return RequestParser::token_t::POST;
		}
		if (strcmp(d1, "HTTP") == 0)
		{
			...
			return RequestParser::token_t::HTTPVERSION;
		}
		...
		if (c1 != NULL)
		{
			value.append(c1);
			m_token = _strdup(value.c_str());
			return RequestParser::token_t::PROPERTYNAME;
		}
	}
	c1 = AcceptRun("\/");
	if (c1 != NULL)
	{
		...
		if (c1 != NULL)
		{
			m_token = _strdup(c1);
			printf("URL ====> %s\n", c1);
			return RequestParser::token_t::URL;
		}
	}
	...
	if (c1 != NULL)
	{
		value.append(c1);
		m_token = _strdup(value.c_str());
		return RequestParser::token_t::QUERYSTRING;
	}
	...
	if (c == NULL)
	{
		return RequestParser::token_t::ENDTOKEN;
	}
	return *c;
}

CHAR* RequestParser::GetUrl()
{
	DWORD dwThreadId = GetCurrentThreadId();
	HEADERMAPLIST::iterator itr;
	for (itr = m_headermap.begin(); itr != m_headermap.end(); itr++)
...
MethodType RequestParser::GetMethod()
{
	HEADERMAPLIST::iterator itr;
	for (itr = m_headermap.begin(); itr != m_headermap.end(); itr++)
	{
