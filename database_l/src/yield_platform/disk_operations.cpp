...
fd_t DiskOperations::open( const Path& path, unsigned long flags )
{
  ...
  wstring pat = L"c:\\temp\\test1.idx";

  wstring pat2 = path.getWidePath();
  ...
  //HANDLE fd = CreateFileW(pat.c_str(), file_access_flags, file_share_flags, NULL, file_create_flags, file_open_flags, NULL);
#else
