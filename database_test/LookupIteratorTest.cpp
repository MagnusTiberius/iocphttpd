...
namespace database_test
{
	...
	TEST_CLASS(LookupIteratorTest)
	{
	public:

		TEST_METHOD(LookupIteratorEmptyDb)
		{
			StringOrder sorder;
			...
			Buffer lower("no0");
			Buffer upper("no1");
			babudb::LookupIterator it_empty(logi, NULL, sorder, lower, upper);

			babudb::LookupIterator it_empty2(logi, NULL, sorder);
		}


		TEST_METHOD(LookupIteratorEmptyIterator)
		{
			StringOrder sorder;
			babudb::LookupIterator it_empty(sorder);
			EXPECT_FALSE(it_empty.hasMore());
		}

		TEST_METHOD(LookupIterator)
		{
			StringOrder sorder;
			...
			{  // Create an ImmutableIndex
				IndexMerger* merger = new IndexMerger(Util::testPath("testdb-testidx"), sorder);
			...
			ImmutableIndex* loadedindex = ImmutableIndex::Load(Util::testPath("testdb-testidx_4.idx"), 4, sorder);
			...
			{  // try empty iterator
				...
				babudb::LookupIterator it_empty(logi, loadedindex, sorder, lower, upper);
				EXPECT_FALSE(it_empty.hasMore());
			}

			{  // only load ImmutableIndex
				babudb::LookupIterator it(vector<LogIndex*>(), loadedindex, sorder);
			...
			{  // now for a range query
				Buffer lower("key1x");
				Buffer upper("key4x");
				babudb::LookupIterator it(logi, loadedindex, sorder, lower, upper);
			...
			{  // and another one that equals the whole range
				Buffer lower2("key");
				Buffer upper2("key5x");
				babudb::LookupIterator it(logi, loadedindex, sorder, lower2, upper2);
			...
			{  // and as a full sweep
				babudb::LookupIterator it(logi, loadedindex, sorder);
...

			{
				...
				Buffer lower2("key");
				Buffer upper2("key3");
				babudb::LookupIterator it2(logi, loadedindex, sorder, lower2, upper2);
